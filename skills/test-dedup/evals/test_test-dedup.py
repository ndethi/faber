...
def test_skill_md_exists() -> None:
    ...
    skill_md = Path("skills/test-dedup/SKILL.md")
    ...
    assert "test-dedup" in content, "SKILL.md must mention skill name"
    print("✓ SKILL.md exists with required fields")


def test_script_exists_and_executable() -> None:
    """Test that main script exists and is executable."""
    script = Path("skills/test-dedup/scripts/test-dedup.py")
...
def test_script_help() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "skills/test-dedup/scripts/test-dedup.py"],
        capture_output=True,
        text=True,
    )
...
def test_determinism() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir1, tempfile.TemporaryDirectory() as tmpdir2:
        ...
        result1 = subprocess.run([
            sys.executable,
            "skills/test-dedup/scripts/test-dedup.py",
            "--input", input_json,
            "--output-dir", tmpdir1
